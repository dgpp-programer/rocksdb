...
void BlockBasedFilterBlockReader::RetrieveBlockDone(AsyncContext &context) {
  ...
  assert(context.read.retrieve_block.contents->GetValue());
  const char* data = nullptr;
  const char* offset = nullptr;
  size_t num = 0;
  size_t base_lg = 0;
  if (!ParseFieldsFromBlock(*context.read.retrieve_block.contents->GetValue(),
      &data, &offset, &num, &base_lg)) {
    // Errors are treated as potential matches
    context.read.key_may_match = true;
    return t->GetAsyncCallback(context);;
  }
  const uint64_t index = context.read.block_offset >> base_lg;
  if (index < num) {
    ...
    if (start <= limit && limit <= (uint32_t)(offset - data)) {
      ...
      context.read.key_may_match = policy->KeyMayMatch(entry, filter);
...
void BlockBasedFilterBlockReader::KeyMayMatchAsync(AsyncContext &context) {
  ...
  table_->RetrieveBlockAsync(context, context.read.retrieve_block.contents.get(),
      cache_filter_blocks());
}
